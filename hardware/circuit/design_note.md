...
- **DDSを発振器として利用**  
  ...
  従来のコイルドライバではアナログ発振回路を用いていましたが、本設計ではDDSを採用しました。これにより、周波数変更の作業がソフトウェアで完結するため、ハードウエアの改修（抵抗交換）不要となり、周波数の変更にも容易に対応できます。
...
# 各部の詳細設計
この設計では機能を、市販の「DDS回路」と設計した「本体回路」で実現しています。  
...
- 電源電圧：3.3V
  - モジュールの仕様としては電源電圧5Vです。初期の設計でDDSを5V動作に設定して動作確認したところ、CPLDのIO電圧が3.3VであるためDDSがCPLDからのコマンドを受け取れないという状況になりました。そのため、3.3V動作としています。
...
### 外部入力回路（未使用） (U2A)
コイルを駆動する信号を外部から入力するための回路です。今回の設計では使用していません。  
差動アンプで受けることで外部の発振器からのノイズ流入を抑えています。  
...
DDSのDAコンバータによる高調波ノイズを抑えるため、1kHz程度のLPFがかかっています。
...
実動作時の消費電流は10mA～20mA（放電率0.01C程度）と小さいため、警告後も直ちに動作停止することはありません。フロントパネルの赤色LED点灯により、バッテリー交換/充電時期をお知らせします。  
...
## 回路図 2/4
<img src="/figures/ER-524D_CoilDriver24_SCH_P2_function.png" alt="CKT_page2" width="800px">

### マルチプレクサ回路
24個のコイルを順次切り替えて駆動するためのマルチプレクサ回路です。この回路図では最大50個のコイルまで駆動できるようにマルチプレクサ回路を組んであります。  
8ch切り替えのアナログスイッチ7個をプラス側とマイナス側で1セットづつ、合計14個使います。それぞれのアナログスイッチにはチャネルを設定する3bitの信号と、チップセレクトがあり、これらを組み合わせて50chの切り替えを実行します。スイッチにはリーク電流が少ないタイプのものを選んでいます。スイッチのリーク電流は内部の寄生ダイオードによる電源からのリークなので直流であることが想定される一方、今回の設計では交流駆動であるため直流リーク電流による測定への影響は小さいと考えています。ただ、スイッチのリーク直流がコイルに流れ、磁場が発生している可能性は否定できません。

## 回路図 3/4

<img src="/figures/ER-524D_CoilDriver24_SCH_P3_function.png" alt="CKT_page3" width="800px">
...
### コイルドライブ出力コネクタ
マルチプレクサからの電流出力を外部に出力するためのコネクタです。50ピンSCSIコネクタ2個で50ch分の信号を外部に出力しています。この設計では24chのドライブですので、その半分だけ、1個のコネクタを使います。  
コネクタの出力ピンは25列上下２段構成になっているので、その上下で1セットのコイルを接続すように設計してあります。  
マルチプレクサのチャネル指定とこの出力コネクタのピン番号の関係は以下のようになっています。ピン番号とチャネル指定番号とは昇順・降順が逆になっているので注意してください。

|MUX channel   |Signal name  |Connector Pin Num|
|---|---|---|
|0  |DRIVE01         |25/50 |
|1  |DRIVE02         |24/49 |
|2  |DRIVE03         |23/48 |
...
<img src="/figures/ER-524D_CoilDriver24_SCH_P4_function.png" alt="CKT_page4" width="800px">

### 同期信号出力回路
同期信号を出力する回路です。ラインドライブ用のIC(U29)を用いてドライブ能力を強化しています。また、絶縁信号の出力を用意するためにフォトカプラを使っています。フォトカプラは同期信号を受け取る装置と本装置の間でGNDループを形成しないようにする目的で導入されています。高耐圧の絶縁用途として設計されていません。  
フォトカプラによる絶縁出力は、信号の受け側に電源が必要になりますので注意してください。

### スイッチ接続回路
ユーザインターフェイス回路からのスイッチ入力および、表示用LEDのドライブを行う回路です。  
スイッチ入力にはチャタリング除去用のCR回路があります。LEDドライブのためにバッファIC(U29)を利用しています。
...
バッテリケースからユーザインタフェース回路までの電源ケーブルについてはこの回路図に記載がありません。現物に合わせて作成をお願いします。バッテリケースからの電源引き出しのケーブルはAWG24程度の太めの線を使用しました。ポリヒューズが動作する電流でケーブルが過熱しない様にケーブルの電流容量を見積もってください。
