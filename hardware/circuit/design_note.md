...
8ch切り替えのアナログスイッチ7個をプラス側とマイナス側で1セットづつ、合計14個使います。それぞれのアナログスイッチにはチャネルを設定する3bitの信号と、チップセレクトがあり、これらを組み合わせて50chの切り替えを実行します。スイッチにはリーク電流が少ないタイプのものを選んでいます。スイッチのリーク電流は内部の規制ダイオードによる電源からのリークなので直流であることが想定される一方、今回の設計では交流駆動なので直流のリーク電流による測定への影響は小さいと考えています。ただ、スイッチのリーク直流がコイルに流れ、磁場が発生している可能性は否定できません。
...
また、マルチプレクサのチャネル指定とこの出力コネクタのピン番号の関係は以下のようになっています。ピン番号とチャネル指定の番号とは昇順・降順が逆になっているので注意してください。
